...
public class MemberRepositoryImpl implements MemberRepositoryCustom{
    ...
    public Page<MemberTeamDto> searchPageSimple(MemberSearchCondition condition, Pageable pageable) {
        QueryResults<MemberTeamDto> results = queryFactory
                ...
                .offset(pageable.getOffset()) // 페이징 시작지점
                .limit(pageable.getPageSize()) // 페이징 한페이지 수
                .fetchResults();// 컨텐츠, 카운트 쿼리 2번 날림 (deprecated 된듯) // 아래 방식 쓰면 해결됨
    ...
    private long getTotal(MemberSearchCondition condition) {
        long total = queryFactory
                ...
                .fetch().size(); // 공식은 이걸 쓰라고 함
        //        .fetchCount(); //이거도 deprecated...?
        return total;
    }
